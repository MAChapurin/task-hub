...
export const TodayTasksWidget = ({ tasks }: { tasks: Task[] }) => {
  ...
  return (
    <Card className="min-h-[50vh] max-h-[80vh] p-4 overflow-x-auto overflow-y-auto relative w-full">
      <h2 className="font-semibold text-3xl mb-4">Сегодняшние задачи</h2>
      <div className="relative min-w-[1440px] flex flex-col h-[calc(80vh-96px)]">
        <div className="absolute top-0 left-0 right-0 z-20 grid grid-cols-24 h-[30px]">
          {Array.from({ length: 24 }).map((_, hour) => (
            ...
            >
              <span
                className={cn(
                  'absolute -top-1 left-1/2 -translate-x-1/2 px-1 bg-muted',
